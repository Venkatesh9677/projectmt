...
const Loginpage = () => {
  ...
  const checklogin = (e) => {
    ...
    if (username.trim() === "" && password === "") {setErrorMessage("Please fill in both username and password.");
     ...
     /*else if (!username.match(alphanumericRegex)) {setErrorMessage("Username should contain only alphanumeric characters.");}*/
     else if (password === "") {setErrorMessage("Please fill in the password.");}
      /*else if (password.includes(" ")) {setErrorMessage("Password should not contain spaces.");} 
      else if (password.length > 15) {setErrorMessage("Password should not exceed 15 characters.");} */
    else if (username === "admin" && password === "1111") {setErrorMessage("Login successful.");
      setTimeout(() => {setErrorMessage("");setusername("");
        setpassword("");}, 5000);}
  ...
  <h1 className="cover-title">Login page</h1>
